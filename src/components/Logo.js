...
import { setIsShowSignin, setIsShowSignup, setIsShowHeader } from "./store";
...
const Logo = () => {
  ...
  const isShowHeader = useSelector((store) => store.github.isShowHeader); 
      ...
      <div className="flex font-semibold gap-4  items-center">
        ...
        {isShowHeader && (
          <div className={`flex ${!isShowHeader && 'hidden'}`}>
            <p>
              Product <RiArrowDropDownLine className="inline -ml-1 text-2xl" />
            </p>
            <p>
              Solution <RiArrowDropDownLine className="inline -ml-1 text-2xl" />
            </p>
            <p>
              Open Source <RiArrowDropDownLine className="inline -ml-1 text-2xl" />
            </p>
            <p>Pricing </p>
          </div>
        )}
      ...
      <div className="flex gap-3">
        <div className="flex items-center border border-white px-10 py-[3px] bg-[#283041] text-white rounded-md  ">
          ...
          <input
            ...
            className="mx-1 bg-[#283041]  outline-none"
          />
          <Searchslash />
        </div>
        ...
        {!isShowHeader && <button onClick={() => signOut()}>Sign out</button>}
      </div>
    </div>
  );
};
