...
const UserInfo = () => {
  ...
  return (
    <div className="py-5 px-5">
      ...
      <button className={false ?"border-b-2 border-black p-2":""} onClick={()=>getFollowers()}>Followers</button>
        <button onClick={()=>getFollowing()}> Following</button>
        <button onClick={()=>getRepos()}>  Repositories</button>
       
      </div>
      {reposData==null ?<div className="flex gap-4 flex-wrap justify-around">
        {followersData && followersData.map(({avatar_url,login})=>{
          return  <ProfilesDetails pic={avatar_url} username={login}/>
        })}
      </div>:

      <div className="flex justify-center flex-col bg-black px-40">
